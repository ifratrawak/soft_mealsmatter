...
def upd_product(request, p_id):
    ...
    if store.owner == request.user:
        if request.method== "POST":
            ...
            prod_form = forms.ProductUpdationFormUser(instance=product)
...
def create_order(request, s_id):
    ...
    if request.method == 'POST':
        order_form = forms.OrderCreationFormUser(request.POST or None, request.FILES or None, user=request.user)
